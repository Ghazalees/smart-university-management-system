...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        department, _ = Department.objects.get_or_create(name="Computer Engineering", code="CE")
        self._create_user("student@university.local", "student", "Student123!", Role.STUDENT, "Demo Student", department, student_number="S-1001")
        self._create_user("professor@university.local", "professor", "Professor123!", Role.PROFESSOR, "Demo Professor", department, employee_number="P-2001")
        self._create_user("staff@university.local", "staff", "Staff123!", Role.ADMINISTRATIVE_STAFF, "Demo Staff", department, employee_number="A-3001")
        self._create_user("president@university.local", "president", "President123!", Role.UNIVERSITY_PRESIDENT, "Demo President", department, employee_number="M-4001")
    ...
    def _seed_roles(self, permissions):
        ...
        for role_name, permission_codes in role_permissions.items():
            role, _ = Role.objects.get_or_create(name=role_name, defaults={"description": f"Default {role_name} role"})
            role.permissions.set([permissions[code] for code in permission_codes])

    def _create_user(self, email, username, password, role_name, full_name, department, student_number="", employee_number=""):
        ...
        Profile.objects.get_or_create(user=user, defaults={"full_name": full_name, "department": department, "student_number": student_number, "employee_number": employee_number})
