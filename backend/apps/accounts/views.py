...
class UserRoleView(APIView):
    ...
    def patch(self, request, pk):
        ...
        UserRole.objects.bulk_create(
            [
                UserRole(user=user, role_id=rid, assigned_by=request.user)
                for rid in role_ids
                if rid not in existing
            ]
        )
        AuditService.record(
            AuditEvent("user.roles_updated", "User", user.pk, {"role_ids": role_ids}),
            actor=request.user,
            request=request,
        )
...
class RoleListView(APIView):
    ...
    def get(self, request):
        return success(
            RoleSerializer(Role.objects.prefetch_related("permissions"), many=True).data
        )
...
class PermissionListView(APIView):
    ...
    def get(self, request):
        return success(PermissionSerializer(Permission.objects.all(), many=True).data)
