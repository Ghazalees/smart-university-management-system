...
class CurrentUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ["id", "username", "email", "first_name", "last_name", "department", "roles", "permissions", "profile", "is_active"]
    def get_permissions(self, obj):
        if obj.is_superuser:
            return ["*"]
        return list(Permission.objects.filter(roles__users=obj).values_list("code", flat=True).distinct())
...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    role_ids = serializers.PrimaryKeyRelatedField(source="roles_input", queryset=Role.objects.all(), many=True, required=False, write_only=True)
    profile = ProfileSerializer(required=False)
    class Meta:
        model = User
        fields = ["id", "username", "email", "password", "first_name", "last_name", "department", "role_ids", "profile"]

    def validate_email(self, value): return value.lower()
...
class UserUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ["email", "first_name", "last_name", "department", "is_active", "profile"]
