...
@pytest.fixture
def roles(db):
    return {name: Role.objects.create(name=name, is_system=True) for name in Role.SYSTEM_ROLES}

@pytest.fixture
def student(db, roles):
    user = User.objects.create_user(username="student", email="student@example.com", password="StrongPass123!")
...
def admin_user(db, roles):
    user = User.objects.create_user(username="adminstaff", email="admin@example.com", password="StrongPass123!")
    Profile.objects.create(user=user)
